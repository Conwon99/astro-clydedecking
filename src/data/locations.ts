...
function describe(seed: LocationSeed): { description: string; intro: string } {
  if (seed.character === "glasgowDistrict") {
    return {
      description: `Composite decking, timber decking and fencing installed and repaired across ${seed.name}, Glasgow, by local decking and fencing specialists.`,
  ...
  return {
    description: `Composite decking, timber decking and fencing installed and repaired across ${seed.name} and the surrounding area by Clyde Decking Solutions.`,
...
export const locations: LocationPage[] = locationSeeds.map((seed) => {
  ...
  return {
    ...seed,
    title: `Decking & Fencing in ${seed.name} | Clyde Decking Solutions`,
    description,
    intro,
